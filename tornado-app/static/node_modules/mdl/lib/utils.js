...
var getProjectData = function(dir) {
    ...
    while (dirs.length && !projectData) {
        if (dirCount++ > 5000) {
            Y.log('Scanned ' + dirCount + ' directories looking for a .mdl.json file, something is probably wrong here..', 'error', 'mdl');
            process.exit(1);
        }
        ...
        dirs.forEach(function(dir) {
            ...
            try {
                ...
                names.forEach(function(name) {
                    ...
                    if (Y.Files.isDirectory(p)) {
                        ...
                        if (name === 'node_modules') {
                            Y.log('Skipping node_modules directory while scanning for .mdl.json', 'warn', 'mdl');
                            return;
                        }
                        childDirs.push(p);
                    }
                ...
                Y.log('Accessing dir (' + dir + ') threw an error', 'warn', 'mdl');
            }
        });
...
var getFileData = function(p, name, file) {
    if (name === file) {
        Y.log('Loading ' + name + ' data from: ' + p, 'info', 'mdl');
        try {
