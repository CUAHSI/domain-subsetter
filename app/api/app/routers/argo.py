...
client = Minio(
    get_settings().minio_api_url, access_key=get_settings().minio_access_key, secret_key=get_settings().minio_secret_key
)
...
async def submit(hucs: Annotated[list[str] | None, Query()], user: User = Depends(current_active_user)):
    ...
    api_instance.submit_workflow(
        namespace=get_settings().argo_namespace, body=parameters(hucs, workflows_name), _preload_content=False
    )
